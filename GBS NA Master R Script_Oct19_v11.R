...
data <- dplyr::tbl_df(years) %>% #specify the data table to summarize
  group_by(Opp.No, Rpt.Brand.Subgroup, Open.Won.Lost, Edge.Region.Name,
           Gbs.Sector.Name, Won.Lost.Year, First.Stage, Industry) %>%
...
#DPUID
closed.pipe$DPUID <- with(closed.pipe, paste(First.Stage, size, new.sl, Sector.Man, Industry,
                                             sep = ""))
...
data.pipe <- dplyr::tbl_df(years.pipe) %>% #specify the data table to summarize
  group_by(Opp.No, Brand.Sub.Group, SSM.Step.Name, Previous.Sales.Stage,
           IMT, GBS.Bus.Unit.Level.2, Industry) %>% #specify which records/variables to keep
...
opp.data$DPUID <- with(opp.data, paste(Create.Stage, size, Service.Line, Sector.IMT, Industry,
                                       sep = ""))
...
closed.cube <- subset(closed.pipe,
                      select=c(Opp.No, DPUID, Create.Stage, Deal.Size, Sector.F, Service.Line,
                                  Size.Type, Industry, TCV, create.m.yr, created, mo.bin, source))
...
closed.mon.crt <- subset(closed.mon.crt, select=c(Opp.No, DPUID, Create.Stage, Deal.Size,
                                               Sector.F, Service.Line, Industry, Size.Type, TCV))
...
open.mon.crt <- subset(open.mon.crt, select=c(Opp.No, DPUID, Create.Stage, Deal.Size, Sector.F,
                                      Service.Line, Industry, Size.Type, TCV))
...
monthly.create.data <- dplyr::tbl_df(comb.data) %>%
  group_by(DPUID, Create.Stage, Deal.Size, Sector.F, Service.Line, Industry, Size.Type) %>%
  filter(Create.Stage != 'Closed') %>%
...
OI.heat.map$create.stage <- with(OI.heat.map, ifelse(create.stage == 'Identified', 'Early Stage',
                                                'Validated or later'))
...
OI.heat.map.2 <- dplyr::tbl_df(OI.heat.map) %>%
  group_by(GBS.Bus.Unit.Level.2, create.month, create.stage) %>%
  summarise(tcv.m = sum(tcv)/1000) %>%
  filter(GBS.Bus.Unit.Level.2 != "")

OI.heat.map.2$GBS.Bus.Unit.Level.2 <- with(OI.heat.map.2,
...
#Heat Map
write.csv(OI.heat.map.F, 'OI heat map data_10122016_v3.csv')
