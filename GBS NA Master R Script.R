...
data$freq <- with(data,
                  ifelse(Edge.Region.Name == 'US' & Gbs.Sector.Name == "", 5,
                         ifelse(Edge.Region.Name == 'US' & Gbs.Sector.Name == 'Unassigned', 5,
                                ifelse(Edge.Region.Name == 'US' & Gbs.Sector.Name == 'UNASSIGNED', 5, 1))))

data.expanded <- data[rep(row.names(data), data$freq), 1:15]
...
#Binding Deal Profiles----------------------------------------------------------------------------
rel.w.dplist <- rbind(rel.w.dp.1, rel.w.dp.2, rel.w.dp.3, rel.w.dp.4, rel.w.dp.5,
...
rel.w.yields <-dcast(rel.w.dplist, Deal.Profile + IMT + Service.Line + Deal.Size + Create.Stage ~ mo.bin, value.var = c('yield'))
...
#Relative Won Yield Curves
write.csv(rel.w.yields, 'Relative Yield Curves 08-29-16 v4.csv', na = "0")
